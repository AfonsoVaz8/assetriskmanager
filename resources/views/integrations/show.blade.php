            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 space-y-8">
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ $integration->provider === \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::SHODAN->value
                                    ? __('Global integration')
                                    : ($integration->tenant?->name ?? __('Global / Not configured')) }}
                            </dd>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        @php($credentials = $integration->safeCredentials())
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @if($integration->provider === \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::SHODAN->value)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("API Key") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'api_key') ? __('Stored securely') : __('Not configured') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Base URL") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'base_url', 'https://api.shodan.io') }}</dd>
                                </div>
                            @elseif($integration->provider === \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::GENERIC_IP_INTELLIGENCE->value)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Base URL") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'base_url', __('Not configured')) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Auth Mode") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ \Illuminate\Support\Str::of(data_get($credentials, 'auth_mode', 'none'))->replace('_', ' ')->title() }}</dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'ip_parameter_name', 'ip') }}</dd>
                                </div>
                            @else
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Tenant ID") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'tenant_id', __('Not configured')) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Client ID") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($credentials, 'client_id', __('Not configured')) }}</dd>
                                </div>
                                <div>
                    ...
                    @if($integration->provider === \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::MICROSOFT_GRAPH->value)
                        <div>
                            <h4 class="text-lg font-semibold text-gray-900 mb-4">{{ __("Policy Settings") }}</h4>
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Trusted Countries") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ implode(', ', data_get($integration->settings, 'trusted_countries', [])) ?: __('Not configured') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Trusted Networks") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ implode(', ', data_get($integration->settings, 'trusted_networks', [])) ?: __('Not configured') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("External Sign-In Detection") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->settings, 'detect_external_signins', true) ? __('Enabled') : __('Disabled') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Unusual Country Detection") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->settings, 'detect_unusual_countries', true) ? __('Enabled') : __('Disabled') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Notify on High Severity") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->settings, 'notify_high_severity', true) ? __('Yes') : __('No') }}</dd>
                                </div>
                            </dl>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        @if(in_array($integration->provider, [
                            \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::SHODAN->value,
                            \App\Domain\ThreatMonitoring\Enums\IntegrationProvider::GENERIC_IP_INTELLIGENCE->value,
                        ], true))
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Last Dispatch") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->sync_state, 'last_dispatched_at') ?: __('Never') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Jobs Dispatched") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->sync_state, 'last_job_dispatch_count', 0) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Last Asset Sync") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->sync_state, 'last_report_synced_at') ?: __('Never') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Last Asset Status") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ data_get($integration->sync_state, 'last_report_status', __('Unknown')) }}</dd>
                                </div>
                            </dl>
                        @else
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Last Sign-Ins Cursor") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900 break-all">{{ data_get($integration->sync_state, 'sign_ins_last_seen_at', __('Not available')) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">{{ __("Last Risk Detections Cursor") }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900 break-all">{{ data_get($integration->sync_state, 'risk_detections_last_seen_at', __('Not available')) }}</dd>
                                </div>
                            </dl>
                        @endif
                    </div>
                        ...
                        <div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm text-left text-gray-500">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-4 py-3 text-gray-500">{{ __("No asset reports have been created yet.") }}</td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                                </table>
                        ...
                        <div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm text-left text-gray-500">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-4 py-3 text-gray-500">{{ __("No threat events synced yet.") }}</td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                                </table>
                        ...
                        <div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm text-left text-gray-500">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-4 py-3 text-gray-500">{{ __("No incidents created yet.") }}</td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                                </table>
