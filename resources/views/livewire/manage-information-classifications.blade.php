<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Manage Information Classifications') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-2xl font-bold mb-4">{{ __('Manage Information Classifications') }}</h2>

                <form wire:submit="save" class="mb-8 space-y-4 bg-gray-50 p-4 rounded">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Name (ex: Secret)') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Hierarchical Level (ex: 3)') }}</label>
                            <input type="number" wire:model="level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if($editingId)
                            <button type="button" wire:click="$set('editingId', null); $set('name', ''); $set('level', '')" class="text-gray-600 hover:underline">
                                {{ __('Cancel') }}
                            </button>
                        @endif
                    </div>
